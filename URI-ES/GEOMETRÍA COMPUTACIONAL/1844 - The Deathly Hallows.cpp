...
// Nome: The Deathly Hallows
// Nível: %d
// Categoria: GEOMETRÍA COMPUTACIONAL
// URL: https://www.urionlinejudge.com.br/judge/es/problems/view/1844
...
typedef pair<Vec3, Vec3> segReta;
...
int main()
{
    ...
    vector<segReta> segs(n);
    vector<aresta> arestas;

    for(auto &it : segs)
        scanf("%lf %lf %lf %lf", &it.first.x, &it.first.y, &it.second.x, &it.second.y), 
            it.first.z = 0, it.second.z = 0;

    for(int i = 0; i < n; i++)
        for(int j = 0; j < n; j++)
            arestas.push_back({distSegmentoSegmento(segs[i].first, segs[i].second, segs[j].first, segs[j].second), {i, j}});
